...
class TopAggregator(Role, metaclass=ABCMeta):
    ...
    def _read_weights_from_plasma(weights_list, dataset_sizes):
        total = 0

        for w in weights_list:
            total += dataset_sizes[w]

            # NOTE: Using object_id_str to access plasma shared memory
            object_id = plasma.ObjectID(w)
            ...
            tres = TrainResult(tensor, dataset_sizes[w])
            self.cache[w] = tres
    ...
    def _aggregate_weights(self, tag: str) -> None:
        ...
        msg, metadata = channel.recv(channel.one_end())
        if not msg:
        ...
        if MessageType.WEIGHTS_LIST in msg:
            weights_list = msg[MessageType.WEIGHTS_LIST]

        # TODO: obtain dataset size of each trainer
        if MessageType.DATASET_SIZES in msg:
            dataset_sizes = msg[MessageType.DATASET_SIZES]

        if weights_list is not None and len(weights_list) > 0:
            logger.debug(f"model weights in Plasma: {weights_list}")
            total = self._read_weights_from_plasma(weights_list, dataset_sizes)
    ...
    def _distribute_weights(self, tag: str) -> None:
        ...
        # send out the notification to scheduler
        logger.debug(f"notifying scheduler to distribute weights")
        msg = {
