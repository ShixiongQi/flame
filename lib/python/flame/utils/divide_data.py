...
class DataPartitioner(object):
    ...
    def __init__(self, data, test_ratio, numOfClass=0, seed=10, isTest=False):
        self.partitions = []
        self.rng = Random()
        self.rng.seed(seed)
...
def select_dataset(rank, partition, batch_size, num_loaders, isTest=False, collate_fn=None):
    """Load data given client Id"""
    partition = partition.use(rank - 1, isTest)
    dropLast = False if isTest else True
    if isTest:
        ...
        num_loaders = min(int(len(partition)/batch_size/2), num_loaders)
    if num_loaders == 0:
        time_out = 0
    else:
        time_out = 60
